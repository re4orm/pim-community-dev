...
final class FetchProductModelRowsFromCodes
{
    ...
    public function __invoke(array $codes, FetchProductAndProductModelRowsParameters $queryParameters): array
    {
        ...
        $rows = array_replace_recursive(
            ...
            $this->getAttributeAsLabel($codes, $valueCollections, $queryParameters->channel(), $queryParameters->locale()),
            $this->getAttributeAsImage($codes, $valueCollections),
            $this->getChildrenCompletenesses($codes, $queryParameters->channel(), $queryParameters->locale()),
            $this->getFamilyLabels($codes, $queryParameters->locale()),
            $valueCollections
        );
...
SQL;
        ...
        $result = [];
        foreach ($rows as $row) {
            $result[$row['codes']] = $row;
        }

        return $result;
    }

    private function getAttributeAsLabel(array $codes, array $valueCollections, string $channel, string $locale): array
    {
        $result = [];
...
SQL;
    ...
    private function getChildrenCompletenesses(array $codes, string $channel, string $locale): array
    {
        $result = [];
        foreach ($codes as $code) {
...
SQL;
    ...
    private function getFamilyLabels(array $codes, string $locale): array
    {
        $result = [];
        foreach ($codes as $code) {
...
SQL;
        ...
        $rows = $this->connection->executeQuery(
            $sql,
            ['codes' => $codes, 'locale_code' => $locale],
            ['codes' => \Doctrine\DBAL\Connection::PARAM_STR_ARRAY]
        )->fetchAll();
    ...
    /**
     * TODO: use recursive CTE when available in Mysql (8.0)
     *
     * @param array $codes
     *
     * @return array
     */
    private function getValueCollection(array $codes): array
    {
        $sql = <<<SQL
            SELECT 
                pm.code,
...
SQL;
        ...
        $result = [];
        foreach ($rows as $row) {
            $result[$row['code']]['value_collection'] = $this->valueCollectionFactory->createFromStorageFormat(
